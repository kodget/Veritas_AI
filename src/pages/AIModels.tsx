      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {selectedModel && (
              <div className="space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-white font-semibold">{models.find(m => m.id === selectedModel)?.name}</p>
                    <p className="text-slate-400 text-sm">{models.find(m => m.id === selectedModel)?.provider}</p>
                  </div>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-400">Status</span>
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${getStatusColor(models.find(m => m.id === selectedModel)?.status || '')}`}>
          ...
          <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700">
            ...
            <div className="space-y-4">
              <button
                ...
                disabled={isTraining}
                className="w-full px-4 py-3 bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-600 text-white rounded-lg font-semibold transition-colors"
              >
                {isTraining ? (
          ...
          <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700">
            ...
            <div className="space-y-4">
              <div className="h-60 overflow-y-auto space-y-3 bg-slate-900 p-4 rounded-lg">
                {chatMessages.length === 0 ? (
                  <div className="text-center text-slate-400 text-sm">
                    <p>Start a conversation with the AI model</p>
                    <p className="text-xs mt-1">Ask about claim patterns, fraud indicators, or model capabilities</p>
                  </div>
                ) : (
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
                />
                <button
                  onClick={sendMessage}
